...
function WallPanel({ position, rotation, w }: { position: [number, number, number]; rotation: [number, number, number]; w: number }) {
  return (
...
/* ------------------------------------------------------------------ */
/* Painting layout — distribute across the 4 walls                     */
/* ------------------------------------------------------------------ */
...
function buildSlots(count: number): Slot[] {
  const eye = 1.9;
  const inset = 0.06;
  const walls: Array<{ len: number; make: (t: number) => Slot }> = [
  ...
  const slots: Slot[] = [];
  // Distribute proportionally around the perimeter
  let assigned = 0;
  walls.forEach((w, i) => {
...
function PaintingMesh({
  ...
  // Fit canvas to image aspect, cap at max size
...
/* ------------------------------------------------------------------ */
/* First-person controller: WASD + PointerLock, clamped to room bounds */
/* ------------------------------------------------------------------ */
...
function FPSController({
  ...
  onNearest: (nearest: { index: number; distance: number } | null) => void;
  paintingPositions: THREE.Vector3[];
}) {
...
export function MuseumScene({
  ...
  onNearestChange: (p: Painting | null) => void;
}) {
  const slots = useMemo(() => buildSlots(paintings.length), [paintings.length]);
  const positions = useMemo(
    () => slots.map((s) => new THREE.Vector3(s.position[0], s.position[1], s.position[2])),
    [slots],
  );
  const nearestIdxRef = useRef<number | null>(null);

  // E key to open modal for nearest painting
  useEffect(() => {
...
function PaintingFrameWithProximity({
  painting,
  slot,
  index,
  nearestIdxRef,
  ...
  slot: Slot;
  index: number;
  nearestIdxRef: React.MutableRefObject<number | null>;
  onClick: () => void;
}) {
